        ...
        <div class="row justify-content-center">
            <div class="col-lg-7">
                <div class="section-title text-center mb-50">
                    <span class="sub-title">{{ __('Get updates on latest Events...') }}</span>

                    <h2 class="title">{{ __('Our Events') }}</h2>
                </div>
            </div>
        </div>
                    ...
                    <div class="services__item-five text-center">
                        <div class="services__icon-five">
                            <div class="icon services-icon icon-medium">
                                    <a href="{{ $event->url }}" class="shine-animate">
                                        {{ RvMedia::image($event->photo, $event->name, 'medium-square') }}
                                    </a>
                            </div>
                        ...
                        <div class="services__content-five">
                            <h2 class="title"><a class="truncate-1-custom" title="{{ $event->name }}" href="{{ $event->url }}">{{ $event->name }}</a></h2>

                            @if ($description = $event->description)
                                <p class="truncate-3-custom">{!! BaseHelper::clean($description) !!}</p>
                            @endif

                            <a href="{{ route('public.events.show', $event->slug) }}" class="btn">{{ __('Read More') }}</a>
                        </div>
                    </div>
                </div>
            @endforeach
