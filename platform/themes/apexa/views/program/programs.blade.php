@php
    Theme::setPageTitle("Programs");
    Theme::layout('full-width');
@endphp

<section class="services__area-seven services__area-home7 services__bg-seven shortcode-services shortcode-services-style-6">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-7">
                <div class="section-title text-center mb-50">
                    <span class="sub-title">{{ __('Get updates on latest Programs...') }}</span>

                    <h2 class="title">{{ __('Our Programs') }}</h2>
                </div>
            </div>
        </div>
        <div class="row justify-content-center gutter-24 services__item-style-2">
            @foreach($programs as $program)
                <div class="col-xl-4  col-md-6">
                    <div class="services__item-five text-center">
                        ...
                        <div class="services__content-five">
                            <h2 class="title"><a class="truncate-1-custom" title="{{ $program->name }}" href="{{ route('public.programs.show', $program->id) }}">{{ $program->name }}</a></h2>

                            @if ($description = $program->description)
                                <p class="truncate-3-custom">{!! BaseHelper::clean($description) !!}</p>
                            @endif

                            <a href="{{ route('public.programs.show', $program->id) }}" class="btn">{{ __('Read More') }}</a>
                        </div>
                    </div>
                </div>
            @endforeach
        ...
        @if ($programs instanceof \Illuminate\Contracts\Pagination\LengthAwarePaginator && $programs->total() > 0)
            <div class="view-more text-center wow animated fadeIn">
                {{ $programs->withQueryString()->links(Theme::getThemeNamespace('partials.pagination')) }}
            </div>
        @endif
    </div>
</section>
